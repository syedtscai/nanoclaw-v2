...
const PROMPT = [
  ...
  '- If `digest` is FALSE → SILENT ingestion run. Do NOT message Zora. Just file facts + write the run log. EXCEPTION: if you filed a CRITICAL signal (importance 4-5: churn / lost deal / major decision / hard conflict with a source:user fact), send an immediate brief alert to Zora (`send_message` to:"zora") naming the fact + entities, then continue.',
