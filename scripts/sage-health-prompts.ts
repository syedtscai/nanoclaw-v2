...
export const HEALTH_PROMPT = [
  ...
  '2. For each active account, recall its signal facts (several varied `mnemon recall` queries — the account name, plus its risks/renewal/friction/blockers): type:renewal-risk, type:risk, type:friction (+ pending friction-candidate), type:blocker, type:exec-ask, type:sentiment, your own type:reconciliation flags, and the most recent src:slack/jira/gmail/krisp/hubspot facts. Note the date: tags to judge recency.',
