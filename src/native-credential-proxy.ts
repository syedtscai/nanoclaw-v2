 ...
 *
 * Reads `process.env` first, then falls back to the `.env` file via
 * `readEnvFile`. The fallback is essential on launchd/systemd installs, where
 * the service runs `node dist/index.js` with only PATH/HOME in its
 * environment — `.env` is never loaded into `process.env`, so a
 * `process.env`-only check would always be false and silently no-op the proxy.
 */
export function nativeCredentialsEnabled(): boolean {
  const fromEnvFile = readEnvFile([NATIVE_CREDENTIALS_FLAG])[NATIVE_CREDENTIALS_FLAG];
  return (process.env[NATIVE_CREDENTIALS_FLAG] ?? fromEnvFile) === 'true';
}
