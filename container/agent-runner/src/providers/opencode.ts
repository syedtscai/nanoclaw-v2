...
export class OpenCodeProvider implements AgentProvider {
  ...
  /**
   * Fresh-session-per-run control (the OpenCode analogue of the Claude
   * provider's transcript rotation / "Lever 1").
   *
   * OpenCode keeps session state in an external SQLite store (XDG_DATA_HOME),
   * not an on-disk transcript whose bytes we can measure — so there is no size
   * to threshold on. But resuming still re-sends the accumulated session every
   * run, so a stateless batch agent (e.g. Iris) that resumes grows its context
   * unboundedly run-over-run — the same cost trap Lever 1 fixes for Claude
   * (which there ballooned to millions of tokens/run). When a per-group rotate
   * cap is configured (`transcriptRotateBytes`), treat it as "start fresh every
   * run": drop the continuation so each run is a clean, flat-cost session. The
   * agent's durable state lives entirely outside the session (mnemon, cursors,
   * run logs), so nothing is lost. Conversational groups leave the cap unset and
   * keep continuity (returns null → resume as normal).
   */
  maybeRotateContinuation(_continuation: string, _cwd: string): string | null {
    const cap = this.options.transcriptRotateBytes;
    if (typeof cap === 'number' && cap > 0) {
      return 'opencode rotation enabled (transcriptRotateBytes set) — stateless batch agent';
    }
    return null;
  }
