...
- **Document extraction** → the base image ships `markitdown` (office/PDF/HTML/CSV → Markdown) in a venv at `/opt/markitdown`, plus `tesseract-ocr` for image OCR. Agents reach these via the `doc-extract` container skill. `markitdown` is `pip`-installed and **outside** pnpm's `minimumReleaseAge` gate — it's pinned exactly via `ARG MARKITDOWN_VERSION`; bump deliberately after checking the PyPI release date. Debian's pip is PEP-668 externally-managed, hence the venv (not a system-wide install).
